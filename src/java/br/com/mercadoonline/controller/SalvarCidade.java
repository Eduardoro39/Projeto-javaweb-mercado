 ...
 */
package br.com.mercadoonline.controller;

import br.com.mercadoonline.DAO.CidadeDAO;
import br.com.mercadoonline.DAO.GenericDAO;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("idcidade").equals("")) {
                if (dao.save(cidade)) {
                    mensagem = "Cidade " + cidade.getNomeCidade() + " cadastrada com sucesso!";
                } else {
                    mensagem = "Problemas ao cadastrar Cidade " + cidade.getNomeCidade() + "!";
                }
            } else {
                cidade.setIdCidade(Integer.parseInt(request.getParameter("idcidade")));
                if (dao.update(cidade)) {
                    mensagem = "Cidade " + cidade.getNomeCidade() + " alterada com sucesso!";
                } else {
                    mensagem = "Problemas ao alterar Cidade " + cidade.getNomeCidade() + "!";
                }
            }
        } catch (Exception ex) {
            System.out.println("Problemas ao salvar Cidade! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
